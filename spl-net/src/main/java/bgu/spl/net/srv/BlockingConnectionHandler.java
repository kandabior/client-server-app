...
import bgu.spl.net.impl.stomp.Client;
...
public class BlockingConnectionHandler<T> implements Runnable, ConnectionHandler<T> {
    ...
    public BlockingConnectionHandler(Socket sock, MessageEncoderDecoder<T> reader, StompMessagingProtocol<T> protocol) {
        this.sock = sock;
        this.encdec = reader;
        this.protocol = (StompMessagingProtocol)protocol;
    ...
    public void run() {
        try (Socket sock = this.sock) { //just for automatic closing
            ...
            protocol.start(id, ConnectionsImp.getInstance());
            ConnectionsImp.getInstance().setId_CH(id,this);
